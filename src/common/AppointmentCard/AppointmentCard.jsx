...
export const AppointmentCard = ({
  ...
  return (
    <div>
      <Button
        variant="primary"
        className="appointmentDesign"
        onClick={() => showMe(appointment)}
      >
        <div>
          <span className="bold">Appointment identification number:</span> {id}
        </div>
        <div>
          <span className="bold">Date:</span> {date}{" "}
          <span className="bold">Hour:</span> {hour}{" "}
          <span className="bold">Artist:</span> {artist_name}
        </div>
        <div className="bold">Customer:</div>
        <div>
          <span className="bold">Name:</span> {user_name}{" "}
          <span className="bold">Surnames:</span> {user_surnames}
        </div>
        <div>
          <span className="bold">Mail:</span> {user_email}{" "}
          <span className="bold">Phone:</span> {user_phone}
        </div>
      </Button>
      ...
      >
        <Modal.Header className="modal-design-header">
          <div>
            <span className="bold">Appointment identification number:</span>{" "}
            {id}
          </div>
          <div>
            <span className="bold">Date:</span> {date}{" "}
        ...
        <Modal.Footer className="modal-design-footer">
          {reduxUserData.credentials.userData.roleId !== 3 && (
            <>
              <div
                onClick={() => showConfirm(appointment)}
      ...
      >
        ...
        <Modal.Footer className="modalConfirm-design-footer">
          <div onClick={() => unsetAppointment({ id })} className="modalButton">
            Delete appointment
          </div>
          <div className="spacer"></div>
          <div onClick={cancelDelete} className="modalButton">
            Return
          </div>
        </Modal.Footer>
      </Modal>
